    ...
    return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-md w-full bg-white p-8 rounded-3xl shadow-xl border border-slate-100 text-center">
                ...
                <h2 className="text-3xl font-bold text-slate-900 mb-2">Payment Not Completed</h2>
                <p className="text-slate-600 mb-6">
                    We couldn&apos;t complete your payment for{' '}
                    <span className="font-semibold">{planName || 'the service'}</span>.
                    No amount has been charged for this attempt.
                </p>

                {transactionId && (
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mb-4 inline-block w-full">
                        <p className="text-xs text-slate-500 uppercase tracking-wide font-semibold mb-1">
                            Reference ID
                        </p>
                        <p className="text-slate-800 font-mono text-sm truncate">{transactionId}</p>
                    </div>
                )}
                ...
                <div className="space-y-3">
                    <button
                        onClick={() => navigate(serviceId ? `/services/${serviceId}` : '/payment', {
                            state: serviceId ? undefined : location.state,
                        })}
